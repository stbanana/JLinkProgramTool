...
def GetPath():
    ...
    if getattr(sys, 'frozen', False):
        # 如果是打包后的单个exe文件运行，'frozen' 属性会被设置
...
jlink_dll_path = os.path.join(GetPath(), 'JLink_x64.dll')
...
def FLashThread(JLinkNo, chip, file, addr, info=None):
    ...
    jlink.set_reset_strategy(pylink.enums.JLinkResetStrategyCortexM3.NORMAL)
...
if __name__ == '__main__':
    ...
    helpmenu.add_command(label="关于", command=lambda: messagebox.showinfo("关于", '作者：yono\n版本：' + __version__ + '\n'))
    ...
    chip = StringVar(value='STM32H743II')
